# Archive Index - 2026-02

> cnc-home 프로젝트 PDCA 문서 아카이브

## Archived Features

| Feature | Match Rate | Archived | Documents |
|---------|:----------:|----------|-----------|
| home-page | 97% | 2026-02-11 | plan, design, analysis, report |
| products-page | 100% | 2026-02-11 | plan, design, analysis, report |
| training-page | 100% | 2026-02-11 | plan, design, analysis, report |
| catering-page | 100% | 2026-02-11 | plan, design, analysis, report |
| b2b-page | 100% | 2026-02-11 | plan, design, analysis, report |
| contact-page | 100% | 2026-02-11 | plan, design, analysis, report |
| seo-optimization | 100% | 2026-02-19 | plan, analysis, report |

## Directory Structure

```
docs/archive/2026-02/
├── _INDEX.md
├── home-page/
│   ├── home-page.plan.md
│   ├── home-page.design.md
│   ├── home-page.analysis.md
│   └── home-page.report.md
├── products-page/
│   ├── products-page.plan.md
│   ├── products-page.design.md
│   ├── products-page.analysis.md
│   └── products-page.report.md
├── training-page/
│   ├── training-page.plan.md
│   ├── training-page.design.md
│   ├── training-page.analysis.md
│   └── training-page.report.md
├── catering-page/
│   ├── catering-page.plan.md
│   ├── catering-page.design.md
│   ├── catering-page.analysis.md
│   └── catering-page.report.md
├── b2b-page/
│   ├── b2b-page.plan.md
│   ├── b2b-page.design.md
│   ├── b2b-page.analysis.md
│   └── b2b-page.report.md
├── contact-page/
│   ├── contact-page.plan.md
│   ├── contact-page.design.md
│   ├── contact-page.analysis.md
│   └── contact-page.report.md
└── seo-optimization/
    ├── seo-optimization.plan.md
    ├── seo-optimization.analysis.md
    └── seo-optimization.report.md
```

## Summary

- **Total Features**: 7
- **Average Match Rate**: 99.6%
- **Total PDCA Documents**: 27
- **Project Period**: 2026-02-10 ~ 2026-02-19
